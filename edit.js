...
function update() {
    ...
    function myProc(obj1) {
        if (obj1.not != "working") {
            for (let key in obj1) {
                document.getElementById(key + "stat").selectedIndex =
                    obj1[key].status;
...
function sendjsondata() {
    ...
    let jsonurl = "https://bencarpenterit.com:3001/submitjson";
    ...
    for (let ride of rides) {
        dataObj[ride] = {};
        dataObj[ride].status = document.getElementById(
            `${ride}stat`
        ).selectedIndex;
...
function customalert(success) {
    if (success == true) {
        ...
        document.getElementById("poststatus").innerHTML =
            'The information has been successfully saved!<p class="poststatus" style="position: absolute; top: 0%; left: 96%" href="#" onclick="hidestatus()"><u><b>X</b></u></p>';
        document.getElementById("poststatusdiv").style.color = "#006e33";
        document.getElementById("poststatusdiv").style.backgroundColor =
            "#2bd487";
        setTimeout(hidestatus, 10000);
    } else {
        document.getElementById("poststatusdiv").hidden = false;
        document.getElementById("poststatus").innerHTML =
            'An ERROR has occurred, please try again!!!<p class="poststatus" style="position: absolute; top: 0%; left: 96%" href="#" onclick="hidestatus()"><u><b>X</b></u></p>';
